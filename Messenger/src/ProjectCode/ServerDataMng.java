...
public class ServerDataMng {
    ...
    // 클라이언트를 방에서 제거하는 메서드 추가
    public void removeClientFromRoom(ObjectOutputStream clientOutStream) {
        String roomName = clientRoomMap.remove(clientOutStream);
        if (roomName != null) {
            ServerRoomMsg roomMsg = chatRoomMap.get(roomName);
            if (roomMsg != null) {
                roomMsg.removeClient(clientOutStream);
            }
        }
    }
}
